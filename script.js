...
function populateContact() {
    try {
        ...
        contactLinks.innerHTML = `
            ...
            ${(contact.instagram || contact.Instagram) ? `<a href="${escapeHtml(contact.instagram || contact.Instagram)}" class="contact-link" target="_blank" rel="noopener noreferrer">
...
function initContactForm() {
    ...
    freshForm.addEventListener('submit', async function (e) {
        ...
        const isWeb3Forms = actionUrl.includes('api.web3forms.com/submit');

        try {
            const response = await fetch(isWeb3Forms ? actionUrl : '/api/send', {
                method: 'POST',
                body: isWeb3Forms ? formData : JSON.stringify({
...
function initResumeDownload() {
    try {
        ...
        resumeBtn.addEventListener('click', function(e) {
            ...
            link.href = 'https://drive.google.com/uc?export=download&id=1GSqBvnOUW3ZrQ3gIC66dvd6ULiwnsCS1';
